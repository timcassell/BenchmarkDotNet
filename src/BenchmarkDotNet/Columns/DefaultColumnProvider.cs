        ...
        private class MetricsColumnProvider : IColumnProvider
        {
            public IEnumerable<IColumn> GetColumns(Summary summary) => summary
                ...
                .SelectMany(report =>
                    ...
                    // Force add Allocated/SurvivedMemoryMetricDescriptor if the memory diagnoser is attached, in case no measurements were able to be made, so we still show the column.
                    .Concat(
                        report.BenchmarkCase.Config.HasSurvivedMemoryDiagnoser() ? new[] { Diagnosers.AllocatedMemoryMetricDescriptor.Instance, Diagnosers.MemoryDiagnoser.SurvivedMemoryMetricDescriptor.Instance }
                        : report.BenchmarkCase.Config.HasMemoryDiagnoser() ? new[] { Diagnosers.AllocatedMemoryMetricDescriptor.Instance }
                        : Array.Empty<IMetricDescriptor>())
                    )
